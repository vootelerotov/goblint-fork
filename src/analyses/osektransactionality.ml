...
struct
  ...
  let pry_d dom_elem = List.fold_left max 0 (List.map (function (LockDomain.Addr.Addr (x,_) ,_) -> (Osek.Spec.pry (GU.IH.find GU.idVar x).vname) 
                                                              | _ -> failwith "This (hopefully5) never happens!"  ) (Mutex.Lockset.ReverseAddrSet.elements dom_elem))
  
  let pry_d' dom_elem r = List.fold_left max 0 (List.map (function (LockDomain.Addr.Addr (x,_) ,_) -> if (GU.IH.find GU.idVar x).vname == r.vname then -1 else (Osek.Spec.pry (GU.IH.find GU.idVar x).vname) 
