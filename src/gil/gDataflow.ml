...
module type ForwardsTransfer = sig
  ...
  val doInstr: instr -> t -> t action
  (** The (forwards) transfer function for an instruction. The 
   * {!currentLoc} is set before calling this. The default action is to 
   * continue with the state unchanged. *)

  val doStmt: stmt -> t -> t stmtaction
  (** The (forwards) transfer function for a statement. The {!currentLoc} 
  ...
  val doGuard: exp -> t -> t guardaction
...
module type BackwardsTransfer = sig
  ...
  val combineStmtStartData: stmt -> old:t -> t -> t option
  ...
  (** The (backwards) transfer function for a branch. The {!currentLoc} is 
   ...
   * {!currentLoc} is set before calling this. If it returns None, then we 
